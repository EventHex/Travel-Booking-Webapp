        ...
        if (response.data.success && response.data.response) {
          ...
          if (destination || goingTo || travelDate || returnDate) {
            filteredData = visaDataArray.filter(visa => {
              // Match country values exactly as they appear in the API
              const fromCountryMatch = !destination || 
                visa.country?.value?.toLowerCase() === destination;
              
              const toCountryMatch = !goingTo || 
                visa.toCountry?.value?.toLowerCase() === goingTo;

              // Date comparison
              let dateMatch = true;
              if (travelDate && returnDate) {
                const searchStart = new Date(travelDate);
                const searchEnd = new Date(returnDate);
                const visaStart = new Date(visa.fromDate);
                const visaEnd = new Date(visa.toDate);
                
                dateMatch = searchStart >= visaStart && searchEnd <= visaEnd;
              }

              console.log(`Checking Visa ${visa._id}:`, {
                fromCountry: {
                  visa: visa.country?.value?.toLowerCase(),
                  search: destination,
                  match: fromCountryMatch
                },
                toCountry: {
                  visa: visa.toCountry?.value?.toLowerCase(),
                  search: goingTo,
                  match: toCountryMatch
                },
                dates: {
                  visaRange: `${visa.fromDate} - ${visa.toDate}`,
                  searchRange: `${travelDate} - ${returnDate}`,
                  match: dateMatch
                }
              });
 ...
 >
      ...
      <div className="max-w-[1300px] w-full mx-auto p-4 rounded-lg">
        ...
        <div className="flex gap-5 flex-col  justify-between md:flex-row p-5 w-full">
         ...
         <SearchInputText    
           value={{ destination: formData.destination, goingTo: formData.goingTo }} 
           dropDownPlace={dropDownPlace}
           dropDownData={citizenOptions}
           onInputChange={handleInputChange}
           data={formData} 
         />
         <SearchInputDate data={formData} onDateChange={handleInputChange} />
            </div>
          ...
          <div className="space-y-4 md:space-y-6">
            {visaOptions.length === 0 ? (
              <div className="text-center py-8">
                <h3 className="text-lg font-medium text-gray-900">No visa options match your search criteria</h3>
